 ...
 * Handles all interactions with Convex realtime cache
 * LAST UPDATED: 2026-01-30 01:47:00
 */
...
console.log('🚀🚀🚀 CLIENT.TS LOADED AT:', new Date().toISOString());

// Note: Convex API will be available after running 'npx convex dev'
let api: any = null;
setTimeout(() => {
  try {
    ...
    // Convex not initialized yet - this is OK
  }
}, 100);
...
class ConvexClient {
  ...
  async syncOrder(order: Order, items: OrderItem[]): Promise<{ success: boolean; error?: string }> {
    ...
    // PROOF LOGGING: Use console.log to bypass any logger issues
    console.log('========================================');
    console.log('🔍 SYNC ORDER INPUT');
    console.log('orderId:', order.id);
    console.log('cancellation_reason value:', order.cancellation_reason);
    console.log('cancellation_reason type:', typeof order.cancellation_reason);
    console.log('cancellation_reason === null:', order.cancellation_reason === null);
    console.log('========================================');
    
    return this.withRetry(async () => {
      // Build the data object - handle null values properly
      const data: any = {
        postgresId: order.id,
      ...
      // CRITICAL FIX: Only add cancellationReason if it has a real value
      if (order.cancellation_reason && order.cancellation_reason !== null) {
        data.cancellationReason = order.cancellation_reason;
      }
      
      if (order.deleted_at) data.deletedAt = order.deleted_at.getTime();
      
      // PROOF LOGGING: Use console.log
      console.log('========================================');
      console.log('🔍 CONVEX DATA BEFORE SEND');
      console.log('orderId:', order.id);
      console.log('has cancellationReason key:', 'cancellationReason' in data);
      console.log('cancellationReason value:', data.cancellationReason);
