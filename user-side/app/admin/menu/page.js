    ...
    <div className="p-6">
      <div className="mb-6 border-b border-black pb-4 flex justify-between items-start">
        <div>
          ...
          <p className="text-black text-xs uppercase tracking-widest">{totalItems} items</p>
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-black p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm" placeholder="Item name" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Price (₹)</label>
                  <input type="number" step="0.01" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Image</label>
                  <input type="file" ref={fileInputRef} accept="image/*" onChange={handleImageUpload} className="hidden" />
                  {imagePreview || formData.image ? (
                    <div className="relative w-full h-[42px] bg-white border border-black flex items-center justify-center">
                      {imagePreview ? (
                        <img src={imagePreview} alt="Preview" className="h-8 w-8 object-cover rounded" />
                      ) : (
                        <ImageIcon size={20} className="text-black" />
                      )}
                      <button type="button" onClick={clearImage} className="absolute right-1 top-1 text-black hover:text-black">
                        <X size={14} />
                      </button>
                    </div>
                  ) : (
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="w-full bg-white border border-black px-3 py-2 text-sm flex items-center justify-center gap-2 hover:border-black">
              ...
              <div>
                <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Category</label>
                <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm">
                  {categories.map((cat) => (<option key={cat} value={cat}>{cat}</option>))}
                </select>
              </div>
              <div>
                <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Description</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm resize-none" rows={2} placeholder="Short description" />
              </div>
              <div>
                <label className="block text-[10px] text-black uppercase tracking-wide mb-2">Available In Zones</label>
                <div className="space-y-1">
                  <button type="button" onClick={selectAllZones} className={`w-full text-left px-3 py-2 text-xs uppercase tracking-wide ${formData.allowedZones.length === 0 ? "bg-white text-black font-bold" : "bg-white text-black hover:text-white"}`}>
                    All Zones {formData.allowedZones.length === 0 && "✓"}
                  </button>
                  {zones?.map((zone) => (
                    <button key={zone._id} type="button" onClick={() => toggleZone(zone._id)} className={`w-full text-left px-3 py-2 text-xs uppercase tracking-wide ${formData.allowedZones.includes(zone._id) ? "bg-blue-600 text-white" : "bg-white text-black hover:text-white"}`}>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button onClick={handleSave} className="flex-1 bg-white text-black py-2 text-xs font-bold uppercase tracking-wide hover:bg-white">{editingItem ? "Update" : "Add"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {!items ? (
        ...
        <div className="bg-white border border-black p-8 text-center">
          <p className="text-black mb-4">No menu items</p>
        ...
        <div className="bg-white border border-black overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-white text-[10px] uppercase tracking-wide">
              <tr>
                <th className="text-left py-3 px-4 text-black">Item</th>
                <th className="text-left py-3 px-3 text-black">Category</th>
                <th className="text-left py-3 px-3 text-black">Zones</th>
                <th className="text-right py-3 px-3 text-black">Price</th>
                <th className="text-right py-3 px-4 text-black">Actions</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                <tr key={item._id} className="border-t border-black/50 hover:bg-white/30">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <MenuItemImage storageId={item.image} alt={item.name} className="w-10 h-10 object-cover rounded" />
                      <div>
                        <p className="font-medium">{item.name}</p>
                        <p className="text-[10px] text-black truncate max-w-[200px]">{item.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-3">
                    <span className="text-[10px] text-black bg-white px-2 py-0.5">{item.category}</span>
                  </td>
                  <td className="py-3 px-3">
                    <span className={`text-[10px] px-2 py-0.5 ${(!item.allowedZones || item.allowedZones.length === 0) ? 'bg-white text-black' : 'bg-blue-900 text-black'}`}>
                  ...
                  <td className="py-3 px-4 text-right">
                    <button onClick={() => handleEdit(item)} className="text-xs text-black hover:text-white mr-3">EDIT</button>
                    <button onClick={() => handleDelete(item._id)} className="text-xs text-black hover:text-black">DELETE</button>
                  </td>
                </tr>
              ))}
            </tbody>
