    ...
    <div className="p-6">
      <div className="mb-6 border-b border-black pb-4 flex justify-between items-start">
        <div>
          ...
          <p className="text-black text-xs uppercase tracking-widest">{tables?.length || 0} tables</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-black p-6 w-full max-w-sm">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm" placeholder="Table 1" />
                </div>
                <div>
                  <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Number</label>
                  <input type="number" value={formData.number} onChange={(e) => setFormData({ ...formData, number: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm" placeholder="1" min="1" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Capacity (seats)</label>
                <input type="number" value={formData.capacity} onChange={(e) => setFormData({ ...formData, capacity: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm" placeholder="4" min="1" max="20" />
              </div>
              <div>
                <label className="block text-[10px] text-black uppercase tracking-wide mb-1">Zone</label>
                <select value={formData.zoneId} onChange={(e) => setFormData({ ...formData, zoneId: e.target.value })} className="w-full bg-white border border-black px-3 py-2 text-sm">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button onClick={handleSave} className="flex-1 bg-white text-black py-2 text-xs font-bold uppercase tracking-wide hover:bg-white">{editingTable ? "Update" : "Add"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {!tables ? (
        ...
        <div className="bg-white border border-black p-8 text-center">
          <p className="text-black mb-4">No tables yet</p>
            ...
            <div key={table._id} className="bg-white border border-black p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => handleEdit(table)} className="text-[10px] text-black hover:text-white px-2 py-1">EDIT</button>
                  <button onClick={() => handleDelete(table._id)} className="text-[10px] text-black hover:text-black px-2 py-1">✕</button>
                </div>
              </div>
              <h3 className="font-medium text-sm">{table.name}</h3>
              <p className="text-black text-xs mt-1">{table.capacity ? `${table.capacity} seats` : 'No limit'}</p>
              {table.zone ? (
                <span className="inline-block mt-2 text-[10px] px-2 py-0.5 bg-blue-900 text-black">{table.zone.name}</span>
              ) : (
