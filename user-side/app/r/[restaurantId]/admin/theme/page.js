    ...
    try {
      ...
      if (restaurant?._id && data.colors) {
        try {
          ...
          setError('Theme applied but failed to save. It will reset on page refresh.');
          setTimeout(() => setError(null), 5000);
        }
      }
    } catch (err) {
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
              ...
              {success && (
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                  ...
                  <p className="text-sm text-green-800">Theme saved successfully! Your custom colors are now applied.</p>
                </div>
              )}
